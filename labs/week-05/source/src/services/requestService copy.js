...
async function fetchSeedRequests() {
  ...
  console.log('fetchSeedRequests', { baseUrl, response });
  if (!response.ok) {
    throw new Error(`ไม่สามารถโหลดข้อมูลตัวอย่างได้: ${response.status} ${response.statusText}`);
  }
...
export async function getRequests(options = {}) {
  ...
  return fetchSeedRequests(); // <-- เปลี่ยนเป็น loadNormalRequests() ในคาบ 5B
}
